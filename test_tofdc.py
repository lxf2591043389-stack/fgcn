...
def resolve_light_ckpt(args, result_dir):
    ...
    if os.path.isfile(cand):
        return cand
    cand = os.path.join(result_dir, "light_best.pth")
    return cand
...
def resolve_heavy_ckpt(args, result_dir):
    ...
    if os.path.isfile(cand):
        return cand
    cand = os.path.join(result_dir, "heavy_best.pth")
    return cand


def main():
    parser = argparse.ArgumentParser(description="TOFDC test")
    ...
    save_dir = os.path.join(result_dir, f"test_results_{args.stage.lower()}_{run_id}")
